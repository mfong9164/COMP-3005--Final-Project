...
* We created the **ER Model** based on the project requirement and the roles of each type of user and the functionality we were going to implement. View the ERD.pdf file to see ER Model: [ERD.pdf](ERD.pdf)
* We **mapped our ER Model into relational tables** by using the methods learned based on cardinality between relationships and foreign keys. View the ERD.pdf file to see ER Model Mapping to Relational Schema: [ERD.pdf](ERD.pdf)
* The database schema was already **normalized to 3NF Normalization**. This means each table’s attributes have no repeating groups or multi-valued attributes, All non-key attributes fully depend on the primary key of that table, and no transitive dependencies (all attributes depend directly on the primary key). We ensured that when creating the entites and schema we would avoid normalization issues in the future. View the ERD.pdf file to see Normalization Analysis: [ERD.pdf](ERD.pdf)
